...
1. Duas formas de entrada:
   ...
   - **Importar arquivo**: clique em **📂 Importar arquivo(s) .xlsx** e escolha o arquivo Excel direto — mesmas 6 colunas, cabeçalho na primeira ou segunda linha (detecta sozinho). Dá pra selecionar **vários arquivos de uma vez**.
2. **Mais de uma lista de material na mesma consolidação.** Os dois botões acima *substituem* o que estiver carregado; os botões **➕ Somar lista colada** e **➕ Somar arquivo(s) .xlsx** *acrescentam* outra lista à consolidação atual. Itens iguais vindos de arquivos diferentes viram **um item só**, com quantidade e massa somadas — é o ponto de carregar várias listas juntas (uma lista por área/subconjunto do projeto, por exemplo).
   - Um bloco **Listas carregadas** aparece logo abaixo dos botões, com uma pastilha por lista (nome + quantas linhas vieram dela) e um 🗑 que **tira aquela lista da consolidação** sem mexer nas outras.
   - Somar uma lista **não perde nada do que já estava ajustado**: as correções feitas à mão na seção 2 e os extras (%) continuam valendo, porque a soma parte das linhas de origem, não dos totais.
   - Com mais de uma lista carregada, a janela de itens de origem (seção 2) mostra **de qual lista** cada linha veio, junto do número da linha — a linha 12 existe em todos os arquivos, então só o número não bastaria.
   - Carregar o mesmo arquivo duas vezes é permitido (as linhas somam de novo), mas a ferramenta avisa e a segunda entra como `nome.xlsx (2)`, pra dar pra remover uma sem a outra.
3. Linhas sem especificação (separadores de seção) ou marcadas como "Esboço Remover" são descartadas automaticamente.
4. A ferramenta agrupa itens idênticos (mesma especificação + descrição + material), somando quantidade e massa.
5. Itens cuja "Especificação" contém **Chapa** viram grupos de corte 2D — chapas retangulares e circulares da mesma espessura e material entram no MESMO grupo (podem ser cortadas da mesma chapa comercial); itens com **Tubo, Perfil, Cantoneira, Barra, Viga** etc. viram grupos de corte 1D; o resto (parafusos, arruelas, curvas/joelhos, grades) aparece na tabela marcado como "Outro" e não entra na otimização — não são itens cortados de chapa/barra, mas podem ser exportados à parte (conteúdo "Itens soltos").
6. A tabela da seção **2 · Itens agrupados** é editável: qualquer célula (Especificação, Descrição, Material, Qtd, Extra %, Massa) pode ser corrigida direto ali, **+ Adicionar item** cria uma linha em branco pra um item que não veio na lista colada/importada, e o 🗑 de cada linha remove um item. Depois de editar, clique em **↻ Reagrupar** — ele relê a tabela inteira e reagrupa/reclassifica do zero, então dois grupos que só existiam separados por causa de um dado errado (ex.: mesma descrição, material digitado diferente) se juntam automaticamente quando a correção fizer os dois baterem. A lista sai sempre em ordem alfabética por Descrição.
7. Cada linha da seção **2 · Itens agrupados** tem um botão **⧉ N** (N = quantas linhas da lista original foram somadas naquele item). Ele abre, logo abaixo da linha, a **janela de itens de origem**: todas as linhas que entraram naquele grupo, com a linha de onde cada uma foi lida (`L12` = linha 12 da planilha/colagem, com o nome da lista embaixo quando há mais de uma carregada), o Item No. original e **todos os campos editáveis um a um** (Item, Especificação, Descrição, Material, Qtd, Massa). Ali também dá pra **remover** uma linha de origem (🗑) ou **adicionar** uma que faltou.
   - A Qtd e a Massa do item agrupado são sempre a **soma** das linhas de origem: editar uma origem atualiza o total na hora. No caminho inverso, digitar um total direto na linha agrupada **rateia** o valor entre as origens quando você sai do campo (proporcional ao que cada uma representava; com uma origem só, vai tudo pra ela).
   - Corrigir Especificação/Descrição/Material **na linha agrupada** corrige junto todas as origens que ainda estavam com o valor antigo — as que você já tinha editado à mão na janela ficam como estão.
   - Se uma linha de origem passar a divergir do grupo (especificação, descrição ou material diferentes), ela ganha o aviso **"sai do grupo"**: no próximo **↻ Reagrupar** ela sai dali sozinha e vai para o grupo que bater com ela (criando um novo, se não existir) — sem arrastar as outras junto. É o que resolve o caso "de dez peças iguais, só uma veio com o material errado".
   - O **↻ Reagrupar** trabalha a partir dessas linhas de origem, não dos totais já somados, então nada se perde no caminho: um item criado à mão com **+ Adicionar item** (que não tem origem nenhuma) continua valendo como uma linha só, com os próprios valores.
8. **Extra (%) — quantidade a mais.** Com a lista já consolidada, cada item agrupado pode levar uma porcentagem a mais — a folga/perda de obra que se compra junto: deu **100 parafusos**, com **10%** de extra vira **110**. Fica na seção **2 · Itens agrupados**, de duas formas:
   - **Item a item**, na coluna **Extra %** da tabela. A coluna **Qtd final** ao lado mostra na hora o total que vai ser comprado e, embaixo, quanto foi acrescentado (peças e massa).
   - **De uma vez**, na barra **Extra (%) — quantidade a mais** no topo da seção: escolha o percentual e a quem aplicar (**todos os itens**, **somente itens soltos** — parafusos, arruelas, curvas... — ou **somente itens cortáveis** — chapas, perfis/tubos) e clique em **+ Aplicar extra**. **Zerar extra** limpa o extra do mesmo grupo escolhido ali.
   - A **Qtd final** é a que vale em tudo o que a ferramenta gera: otimização de corte (mais peças podem significar mais uma chapa/barra comercial), pré-visualização e todas as exportações. A **massa sobe na mesma proporção**, e quantidade inteira é sempre **arredondada para cima** (3 peças com 10% viram 4, não 3,3).
   - A Qtd que continua editável na tabela é a **base** (a consolidada, sem extra) — o extra nunca é somado dentro dela, então dá pra mudar de ideia sem perder o número original. O percentual também **sobrevive ao ↻ Reagrupar**: ele viaja junto com as linhas de origem; se linhas com extras diferentes acabarem no mesmo grupo, vale o maior.
   - Processar/importar uma lista nova zera todos os extras.
9. Seção **3 · O que você vai gerar?** — escolha o **Conteúdo** aqui antes de descer a página: as seções de otimização de corte (**4 · Chapas** e **5 · Perfis/Tubos**) só aparecem quando o conteúdo escolhido é o **"Resumo de corte"** (o único que depende de escolher chapa/comprimento comercial); nos outros conteúdos (lista consolidada em m²/m, compactada, itens soltos) essas seções ficam escondidas, mesmo que existam grupos cortáveis na lista.
10. Nas seções 4/5 (quando aparecem), marque os tamanhos comerciais permitidos de cada grupo cortável (chapa: um por vez; barra: pode marcar mais de um ao mesmo tempo — veja "Barras com mais de um comprimento" abaixo) — o resultado (quantas comprar, aproveitamento, desenho do corte) atualiza na hora. O botão **▾ Minimizar** de cada card recolhe só aquele grupo; **▾ Minimizar todas as espessuras / todos os perfis**, no topo de cada seção, recolhe (ou expande) todos de uma vez.
11. **⇩ Excel compactado + resumo de corte**, na seção 2 — planilha própria com a lista agrupada e um resumo (chapas/barras comerciais necessárias, aproveitamento, massa).
12. Seção **6 · Exportar**, no fim da página — escolha o **Formato** e clique em **👁 Pré-visualizar lista** pra ver exatamente como as linhas vão ficar organizadas (nas mesmas abas/ordem que o arquivo real vai sair) antes de gastar tempo gerando e baixando; quando o resultado parecer certo, clique em **⇩ Gerar Excel**. É uma lista de material do projeto: se o item faz parte da obra, ele aparece na exportação, mesmo os que não passam pela otimização de corte (parafusos, arruelas, curvas etc.) — a única coisa que muda entre "consolidado" e "resumo de corte" é como as chapas e os perfis/tubos são quantificados (por m²/m ou por chapa/barra comercial a comprar); os itens soltos entram do mesmo jeito nos dois. As quantidades exportadas já incluem o **Extra (%)** do passo 8 — a pré-visualização avisa quando algum item está com extra.
